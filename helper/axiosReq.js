...
export const extractText = async (baseImage, setStatus, setOutputData) => {
  ...
  try {
    ...
    const { textAnnotations, fullTextAnnotation } = extractedImage.data.info.ocr.adv_ocr.data[0];
    setOutputData({
      type: "text",
...
export const addOverlay = async (baseImage, overlay, setStatus, setOutputData) => {
  setStatus("loading");
  try {
    const overlayedImage = await axios.post("/api/addOverlay", { baseImage, overlay });
